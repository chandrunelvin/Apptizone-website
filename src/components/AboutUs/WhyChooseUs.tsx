...
const whyChooseCards = [
  {
    ...
    body: 'We focus on measurable growth with more traffic, quality leads, and better ROI.',
  },
  {
    ...
    <section className="relative w-full overflow-hidden bg-[#ecd65d] py-16 lg:py-20">
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1530px] px-6 md:px-10 lg:px-16">
        <TextReveal
          as="h2"
          text="WHY CHOOSE US"
          className="[text-shadow:6px_6px_1px_#201f51] mb-12 text-center font-['Black_Han_Sans',Helvetica] font-normal text-[#4f4298] text-[clamp(42px,7vw,120px)] tracking-[0] leading-[1.01] uppercase sm:whitespace-nowrap md:mb-16"
          delay={40}
        />
        ...
        <div className="relative flex min-h-[556px] flex-col items-center justify-center gap-8 lg:flex-row lg:gap-10 xl:gap-16">
          <div className="flex max-w-[420px] flex-1 flex-col gap-6 md:gap-8 xl:gap-14">
            {whyChooseCards.slice(0, 2).map((card, i) => (
              <ScrollReveal
                key={i}
                className={`relative w-full max-w-[405px] rounded-[30px] bg-white p-6 shadow-md transition-transform duration-300 hover:-translate-y-2 md:p-8 ${card.rotate}`}
                delay={70 + i * 70}
                distance={28}
                blur={8}
              >
          ...
          <ScrollReveal className="w-full max-w-[340px] flex-shrink-0 md:max-w-[420px] lg:w-[clamp(320px,30%,556px)]" delay={140} distance={32} blur={10} scale={0.95}>
